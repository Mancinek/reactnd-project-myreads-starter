		...
		if (query) { // if query is typed into search (query == true) then state is set depending on books returned by api response
			BooksAPI.search(query.trim()).then((booksSearchResult) => {
				/*  if error is returned from search (there is no search results and it get object {error: "empty query", items: Array(0)})
					then state is set to empty array and displays nothing
				*/
				if (booksSearchResult.error) {
					...
					*/
					...
					// after setting shelfs for search results, the state of search page is updated (and books are sorted by title)
					this.setState({books: booksSearchResult.sort(sortBy('title'))})
				}
			})
		} else { // else, if query is not typed or deleted, the state is set to empty array and no books are displayed
			this.setState({books : []})
		}
	}
				...
				<div className="search-books-results">
					<ol className="books-grid">
						{this.state.books.map(book => {
							...
							const checkedImage = book.imageLinks ? book.imageLinks.smallThumbnail : 'Unknown cover' // if imageLinks is undefined, then only default gray background is visible
							return (
								<li key={book.id}>
									<Book 
